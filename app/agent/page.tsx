...
export default function Page() {
  ...
  const onConnectButtonClicked = useCallback(async () => {
    ...
    const url = new URL(
      process.env.NEXT_PUBLIC_CONN_DETAILS_ENDPOINT ?? "/api/connection-info",
  ...
  return (
    <main data-lk-theme="default" className="h-screen flex items-center">
      <div className="w-full grid grid-rows-[64px_1fr_8px] lg:border border-white/20 h-full min-h-dvh lg:max-w-5xl mx-auto lg:min-h-[640px] lg:max-h-[640px] rounded-2xl px-4">
        <header className="border-b border-white/20">
          <div className="py-4 px-2 flex items-center justify-between">
            <p className="text-center w-full">AI Translator</p>
          </div>
        </header>
        <LiveKitRoom
...
function SimpleVoiceAssistant(props: {
  ...
  return (
    <div
      className={`flex flex-col items-center pt-12 justify-center ${
        state === "disconnected" ? "opacity-10" : "opacity-100"
        ...
        {props.agentState !== "disconnected" &&
          props.agentState !== "connecting" && (
            <motion.div
              ...
              className="flex h-8 absolute left-1/2 -translate-x-1/2 items-center justify-center"
            >
